...
const fixturesDir = join(root, 'test', 'fixtures-modern');
...
const fixtures = readdirSync(fixturesDir)
  ...
  .filter((f) => Number(/ModernJdk(\d+)/.exec(f)?.[1] ?? 0) <= maxRelease)
...
for (const fx of fixtures) {
  ...
  const need = Number(/ModernJdk(\d+)/.exec(name)?.[1] ?? 0);
  if (need > jdkMajor) {
    skip++;
    console.log(`SKIP ${name} (needs JDK ${need}, have ${jdkMajor})`);
    continue;
  }
  ...
  if (res.ok) {
    pass++;
    console.log(`PASS ${name} (JDK ${need})`);
  } else if (res.skipped) {
    skip++;
    console.log(`SKIP ${name} (${res.why})`);
  } else {
    fail++;
...
function runOne(fx, name, release) {
  ...
  const c1 = javac(join(work, 'orig'), [fx], { release });
  if (c1.status !== 0)
